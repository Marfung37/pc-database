...
export function generateSetupIDPrefix(pc: number, oqb: boolean, leftover: Queue, build: Queue, coverPattern: string, fumen: Fumen): SetupID {
  ...
  currentBitOffset += 2n;
  tempPackedNum |= BigInt(hashCoverPattern(coverPattern)) << ((BigInt(byteLength) * 8n) - currentBitOffset);
