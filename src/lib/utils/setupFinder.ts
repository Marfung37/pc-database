import type { Result, Setup, Statistic, SetupVariant, Kicktable, Queue, SetupID, HoldType } from '$lib/types';
...
export async function setupFinder(pcNum: number, queue: Queue, previousSetup: SetupID | null = null, kicktable: Kicktable = "srs180", hold_type: HoldType = "any"): Result<Setup[]> {
  ...
  for (let setup of setups) {
    ...
    if(!subStringSet(queue.slice(0, setup.build.length + 1), setup.build))
      continue;
