...
function sortQueues(queues: Iterable<string>): string[] {
    ...
    const queueSetValues = (q: string) => parseInt(q.split('').map(p => pieceValues[p]).join(''), 10);
...
function makeModifierTree(
    ...
    while (index < modifier.length) {
        ...
        // Opening parentheses
            ...
            const [subTree, i] = makeModifierTree(modifier, index + 1, depth + 1, true) as [ModifierTree, number];
...
function handleOperatorInModifier(currBool: boolean, newBool: boolean, operator: string, modifierType: string): boolean {
    ...
    switch (operator) {
        ...
        default:
            ...
            const errorPrefix = "Something went wrong when parsing leading to not catching no operator before a ";
...
function checkModifier(queue: string, modifierTree: ModifierTree): boolean {
    ...
    for (const modifierPart of modifierTree) {
        if (Array.isArray(modifierPart)) {
            ...
            const [newModifierPart, subQueue, negate] = handlePrefixesInModifier(modifierPartStr, queue);
        ...
        // Handle the modifiers
            ...
            // Count modifier
            const countModifierMatchObj = newModifierPart.match(/^([\[\]TILJSZO*]+)([<>]|[<>=!]?=)(\d+)$/);
            ...
            // Before modifier
                ...
                const beforeModifierMatchObj = newModifierPart.match(/^([\[\]TILJSZO*]+)<([\[\]TILJSZO*]+)$/);
    ...
    while (index < extendedSfinderFormatPieces.length) {
        ...
        // Handle modifier
            ...
            if (index + modifierLength + 1 === extendedSfinderFormatPieces.length) {
                throw new Error(
                    `Modifier didn't close '${extendedSfinderFormatPieces.slice(index)}'`
                );
            }
...
function extendPieces(customInput: string | string[] = process.argv.slice(2)): Iterable<string> {
    ...
    for (const extendedSfinderPieces of allExtendedSfinderPieces) {
        ...
        const [queuesPart] = handleExtendedSfinderFormatPieces(
