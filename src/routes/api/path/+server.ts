...
export const GET: RequestHandler = async ({ url, locals: { supabase } }) => {
  ...
  const bucketFilename = generateBucketPathFilename(
    setupid as SetupID,
  ...
  // decompress the file
  const { data: decompressedData, error: decompressError } = await decompressPath(
