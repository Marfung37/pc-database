  ...
  {#if setup.solve_pattern}
    ...
    >
      ...
      <input
        ...
        name="wantedSaves"
        type="text"
        class="focus:shadow-outline bg-base-300 block max-w-64 grow appearance-none rounded border border-gray-300 text-lg leading-tight shadow hover:border-gray-400 focus:outline-none"
    ...
    >
      <div class="flex flex-wrap items-center gap-2">
        ...
        <div>
          <button
            type="submit"
            class="text-base-300 flex w-full cursor-pointer justify-center rounded-md border border-transparent bg-indigo-600 px-4 py-2 font-medium shadow-sm hover:bg-indigo-700 focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:outline-none"
            disabled={loading}
          >
            {loading ? m.loading() : m.btn_submit()}
          </button>
